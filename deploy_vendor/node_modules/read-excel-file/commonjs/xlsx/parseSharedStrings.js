...
var _parseSharedString = require("./parseSharedString.js");
/**
 ...
 */
function parseSharedStrings(content, parseXmlStream) {
...
function onOpenTagInSharedStrings(tagName, attributes, state) {
  if (tagName === 'si') {
    state.si = (0, _parseSharedString.createInitialStateInSharedString)();
  } else if (state.si) {
    (0, _parseSharedString.onOpenTagInSharedString)(tagName, attributes, state.si);
  }
}
function onCloseTagInSharedStrings(tagName, state) {
  if (tagName === 'si') {
    ...
    (0, _parseSharedString.onCloseTagInSharedString)(tagName, state.si);
  }
}
function onTextInSharedStrings(text, state) {
  if (state.si) {
    (0, _parseSharedString.onTextInSharedString)(text, state.si);
  }
}
